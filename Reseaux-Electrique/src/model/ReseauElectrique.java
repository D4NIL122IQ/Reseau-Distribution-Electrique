...
/**
 * Classe représentant un réseau électrique complet. Elle gère les listes de
 * maisons, de générateurs et les connexions qui les lient. Cette classe permet
 * de manipuler la structure du réseau (ajout/suppression) tout en assurant la
 * cohérence des charges électriques. * @author Anis Hammouche, Danil Guidjou
 */
public class ReseauElectrique {
	...
	/**
	 * Ajoute une maison au réseau ou met à jour sa consommation si elle existe
	 * déjà. En cas de mise à jour, la charge du générateur connecté est recalculée
	 * automatiquement.
	 * 
	...
	/**
	 * Ajoute un générateur au réseau ou met à jour sa capacité maximale s'il existe
	 * déjà.
	 * 
	...
	/**
	 * Crée une connexion entre une maison et un générateur existants. Met à jour la
	...
	/**
	 * Ajoute un objet connexion directement au réseau et met à jour la charge du
	 * générateur.
	 * 
	...
	/**
	 * Supprime une connexion spécifique et déduit la consommation de la maison de
	...
	/**
	 * Valide la conformité du réseau selon les règles métier. Un réseau est valide
	 * si chaque maison possède exactement une connexion.
	 * 
	 * @return Un message décrivant les problèmes détectés, ou null si le réseau est
	 *         conforme.
	 */
	public String validerReseau() {
		...
		if (maisons.isEmpty()) {
			...
			problemes.append("Liste MAISON vide\n");
			return problemes.toString();
		}

		if (gens.isEmpty()) {
			System.out.println("  -> Reseau vide, validation necessaire.");
			problemes.append("Liste GENERATEUR vide\n");
			return problemes.toString();
		}
		...
		for (Maison m : maisons) {
			...
			if (count != 1) {
				...
				problemes.append(m.getNomM()).append(" (probleme: ").append(typePb).append(")\n");
			}
		}
